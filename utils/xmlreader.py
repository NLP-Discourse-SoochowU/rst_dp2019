...
def combineparse2sent(sent, parse):
    ...
    while parse:
        ...
        word = tokenlist[tidx].decode('utf-8').replace(' ','')
    ...
    with open(fconll, 'w') as fout:
        for sent in sentlist:
            for token in sent.tokenlist:
                line = str(sent.idx) + '\t' + str(token.idx) + '\t' + token.word.decode('utf-8') + '\t' + token.lemma.decode('utf-8') + '\t' + str(token.pos) + '\t' + str(token.deptype) + '\t' + str(token.headidx) + '\t' + str(token.nertype) + '\t' + str(token.partialparse) + '\n'
